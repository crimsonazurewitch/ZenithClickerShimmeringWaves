...
for d = .5, 2.5, .5 do
    addText({ COLOR.LD, "ZENITH CLICKER" }, 40 + d, 20 + d)
    addText({ COLOR.LD, "SHIMMERING WAVES" }, 140 + d, 95 + d)
end
addText("ZENITH CLICKER", 40, 20)
addText("SHIMMERING WAVES", 140, 95)
...
function scene.draw()
    ...
    gc_mDraw(icon, -300, 120, -0.3, kx, ky)
    gc_draw(AboutText)
    gc_draw(DevNoteText, 0, 325 - DevNoteText:getHeight() * (.68 / 2), 0, .68, .68, 1000, 0)
...
scene.widgetList = {
    ...
    WIDGET.new {
        name = 'SW_About', type = 'button',
        pos = { 0, 0 }, x = 80, y = 880, w = 200, h = 60,
        color = { .15, .2, .3 },
        sound_hover = 'menutap',
        fontSize = 30, text = "    Shimmer", textColor = 'LC',
        onClick = function() love.keypressed('tab') end,
    },
}
