...
function scene.overDraw()
    ...
    if not GAME.invisUI then
        ...
        if M.DP == 0 then
            gc_setColor(GAME.playing and GAME.life > safeHP and COLOR.L or GAME.noHit and COLOR.V or COLOR.R)
            gc_mRect('fill', 800, 440, 1540 * GAME.lifeShow / GAME.fullHealth, 10)
            if GAME.playing and not GAME.noHit then
                gc_setColor(COLOR.LD)
            ...
            elseif GAME.playing and GAME.noHit then
                gc_setColor(.478, .478, .478, .2)
                gc_mRect('fill', 800, 440 - 2, 1540 * GAME.dmgTime / GAME.fullHealth, 3)
                gc_setColor(.872, 0, 0, .2)
                gc_mRect('fill', 800, 440 + 2, 1540 * GAME.dmgWrong / GAME.fullHealth, 3)
                gc_setColor(1, 0, 0, .1262)
                gc_mRect('fill', 800, 440 + 2, 1540 * (GAME.dmgWrong + GAME.dmgWrongExtra) / GAME.fullHealth, 2)
            end
        else
            ...
            gc_setColor(GAME.playing and GAME.life2 > safeHP and COLOR.L or GAME.noHit and COLOR.V or COLOR.R)
    ...
    if not GAME.invisUI then
        ...
        if M.AS > 0 and M.EX == 0 and not GAME.graceless then
            for i = 1, #Cards do
    ...
    if GAME.noHit then
        gc_replaceTransform(SCR.origin)
        gc_setColor(.626, .153, .769, (GAME.playing and .262 or .626) * (M.EX > 0 and .62 or .42))
        gc_draw(TEXTURE.transition, 0, 0, 0, .42 / 128 * SCR.w, SCR.h)
        gc_draw(TEXTURE.transition, SCR.w, 0, 0, -.42 / 128 * SCR.w, SCR.h)
    end
...
scene.widgetList = {
    ...
    WIDGET.new {
        ...
        onPress = function(k)
            ...
            elseif k == 3 or kbIsDown('lshift', 'rshift') then
                SFX.play('menuhit1')
                SCN.go('panel', 'none')
            else
                PieceSFXID = (PieceSFXID or 0) % 11 + 1 -- mod (Pieces + All Clear) + 1
                if PieceSFXID <= 10 then
                    local piece = ('zsjltoiprg'):sub(PieceSFXID, PieceSFXID)
                    SFX.play(piece, 1, 0, Tone(6))
                else
                    SFX.play('allclear')
                end

                for i = 1, 10 do -- Pieces Z -> G (Total: 10)
