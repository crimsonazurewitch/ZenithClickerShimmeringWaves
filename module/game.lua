...
function GAME.getComboZP(list)
    ...
    if m.DH then zp = zp * 1.2 elseif m.rDH then zp = zp * (m.rIN and 2.2 or 1.62) end
    if m.IN then zp = zp * 1.2 elseif m.rIN then zp = zp * 1.55 end
    if m.AS then zp = zp * .85 elseif m.rAS then zp = zp * 1.05 end
    if m.DP then zp = zp * .95 elseif m.rDP then zp = zp * (m.rEX and 1.75 or 2.1) end
...
function GAME.erasingYou(str, density)
    local output = {}
    for i = 1, #str do
        local next = str[i]

        -- next element is string (i.e. NOT color)
        if type(next) == "string" then 

            -- split the string (in case the element is longer than one letter
            local nextLetters = next:atomize()

            -- for each letter in the current element
            for j = 1, #nextLetters do

                -- roll chance for censorship
                -- can also add other "safe" characters apart from ["]
                if nextLetters[j] ~= '"' and ' ' and MATH.roll(density) then
                    ins(output, "#") 
                else
                    ins(output, nextLetters[j])
                end
            end

        -- next element isnt a string (i.e. a color), pass unchanged
        else
            ins(output, next)
        end
    end
    ...
    if mode == 'ingame' then
        ...
        if not GAME.anyRev and not TABLE.find(list, 'DP') then
            ...
            if comboText then
                ...
                if GAME.ryoshu then 
                    return GAME.erasingYou(fstr, censorPercent)
                else
                    return fstr
                end
            end
...
function GAME.genQuest()
    repeat
        ...
        for _ = 1, questCount do
            ...
            if p and not GAME.graceless then
                if p > 1 then
        ...
        if #combo >= 4 and not GAME.graceless then
            local pwr = #combo * 2 - 7
...
function GAME.upFloor()
    ...
    if GAME.floor >= 10 then
        ...
        if GAME.gigaspeed then
            ...
            if not STAT.badge.speedrun_3 then
                _t = 0
                for id in next, MD.name do if rawget(BEST.speedrun, 'u' .. id) then _t = _t + 1 end end
                if _t >= #MD.deck then IssueSecret('speedrun_3') end
            end
...
function GAME.refreshLifeState()
    ...
    if oldState ~= newState then
        ...
        if newState == 'danger' and not GAME.noHit then
            SFX.play('hyperalert')
        end
    end
end
...
function GAME.commit(auto)
    ...
    if correct then
        ...
        if dblCorrect then
            attack = GAME.graceless and attack * 2 or attack * 3
            xp = GAME.graceless and attack * 2 or xp * 3
            GAME.chain = GAME.chain + 1
        ...
        if M.DP > 0 then
            ...
            if M.DP == 2 and not GAME.noHit then
...
function GAME.start()
    ...
    -- Player
    GAME.life = (GAME.noHit and 1) or 20
    GAME.fullHealth = (GAME.noHit and 1) or 20
    ...
    GAME.atkBuffer = 0
    GAME.atkBufferCap = 8 + (M.DH == 1 and M.NH < 2 and 2 or GAME.graceless and 1e99 or 0)
    GAME.shuffleMessiness = false
    ...
    -- rDP
    GAME.onAlly = false
    GAME.life2 = GAME.noHit and 1 or 20
    ...
    if M.DP > 0 then
        GAME.life = GAME.noHit and 1 or 15
        GAME.life2 = GAME.noHit and 1 or 15
        GAME.fullHealth = GAME.noHit and 1 or 15
    end
...
function GAME.finish(reason)
    ...
    if GAME.totalQuest > 2.6 then
        ...
        if not STAT.badge.mastery_3 then
            _t = 0
            for id in next, MD.name do if BEST.highScore['u' .. id] >= Floors[9].top then _t = _t + 1 end end
            if _t >= #MD.deck then IssueSecret('mastery_3') end
        end
        ...
        elseif GAME.comboStr == 'rGV' then
            ...
            if GAME.nightcore and URM then SubmitAchv('sw_Terminal', GAME.roundHeight) end
        ...
        elseif GAME.comboStr == 'rDP' then
            ...
            if GAME.invisUI and URM then SubmitAchv('sw_Blinding', GAME.roundHeight) end
        ...
        elseif GAME.comboStr == 'ASDHEXGVINMSNHVLrDP' then
            ...
            if URM and GAME.comboStr == 'rVL' and GAME.fastLeak then
                SubmitAchv('sw_Inhumane', GAME.roundHeight)
            elseif URM and GAME.comboStr == 'rIN' and GAME.invisCard then
                SubmitAchv('sw_Blackout', GAME.roundHeight)
            elseif URM and GAME.comboStr == 'rEX' and GAME.closeCard then
                SubmitAchv('sw_Resort', GAME.roundHeight)
            elseif URM and GAME.comboStr == 'rDH' and GAME.slowmo then
                SubmitAchv('sw_Process', GAME.roundHeight)
            elseif GAME.comboStr == 'DHVLrDPrIN' and GAME.ryoshu then
                SubmitAchv('sw_muga', GAME.roundHeight)
            elseif GAME.comboStr == 'INrDH' and GAME.ryoshu then
                SubmitAchv('sw_ZCObfuscated', GAME.totalQuest)
            elseif GAME.comboStr == 'DHrMSrVL' and GAME.graceless then
                SubmitAchv('sw_niflhel', GAME.roundHeight)
            elseif GAME.comboStr == 'DHMSrASrNH' and GAME.graceless then
                SubmitAchv('sw_angelicide', GAME.peakRank)
            elseif GAME.comboStr == 'DP' and GAME.noHit then
                SubmitAchv('sw_checkpoint1', GAME.roundHeight)
            end
