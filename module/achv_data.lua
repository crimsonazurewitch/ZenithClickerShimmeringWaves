...
-- Order of the mods in "id" goes from A to Z, followed by Reverses/URMs.
-- [[AS DH DP EX GV IN MS NH VL // rDP rAS rDH rEX rGV rIN rMS rNH rVL // uAS uDH uDP uEX uGV uIN uMS uNH uVL]]
...
Achievements = {
    ...
    { -- Term
        ex = true,
        id = 'sw_Terminal',
        name = "Terminal Velocity, Act II",
        desc = [[HFD with uGV and Nightcore (Z)]],
        quote = [["This might not end well."]],
        rank = floorRank(1, 3, 5, 6, 7, 8, 1200), -- this is a skill issue
        hide = function() return GAME.completion.GV == 0 end,
        credit = "@The_111thBlitzer",
    },
    { -- Black
        ...
        id = 'sw_Blackout',
        name = "Pure Blackout",
        desc = [[HFD with uIN and Invisible Cards (O)]],
        quote = [["Nothing of what you have seen is believable."]],
        rank = floorRank(1, 3, 5, 7, 8, 9, 10),
        hide = function() return GAME.completion.IN == 0 end,
        credit = "@The_111thBlitzer",
    },
    { -- Last
        ...
        id = 'sw_Resort',
        name = "Last Resort",
        desc = [[HFD with uEX and Close Cards (I)]],
        quote = [["The last plot to quell the rebellion."]],
        rank = floorRank(1, 2, 3, 225, 4, 375, 5),
        hide = function() return GAME.completion.EX == 0 end,
        credit = "@The_111thBlitzer",
    },
    { -- Reunion
        ex = true,
        id = 'sw_Blinding',
        name = "Blinding Reunion",
        desc = [[HFD with uDP and Invisible UI (T)]],
        quote = [["Separated by destiny but reunited once again upon blinding lights."]],
        rank = floorRank(1, 2, 3, 4, 5, 525, 600),
        hide = function() return GAME.completion.DP == 0 end,
        credit = "@Echazarel",
    },
    { -- Inhumane
        ex = true,
        id = 'sw_Inhumane',
        name = "Inhumane Determination",
        desc = [[HFD with uVL and Fast Leak (L))]],
        quote = [["Carnage you initiated leaves behind corpses of what you called monsters."]],
        rank = floorRank(1, 3, 5, 7, 8, 9, 10),
        hide = function() return GAME.completion.VL == 0 end,
        credit = "@Echazarel",
    },
    { -- Process
        ex = true,
        id = 'sw_Process',
        name = "Hellmaster's Process",
        desc = [[HFD with uDH and Slow-mo (S))]],
        quote = [["Relive how it feels being the hellmaster all again. Endless hours..."]],
        rank = floorRank(1, 3, 5, 7, 9, 10, 2000),
        hide = function() return GAME.completion.DH == 0 end,
        credit = "@Echazarel",
    },
    { -- :3c
        ex = true,
        id = 'sw_ZCObfuscated',
        name = "Zenith Clicker: Obfuscated",
        desc = [[Quests passed with rDH IN and Redaction (R)]],
        quote = [[Hidden, censored and omitted values.]],
        rank = numberRank(0, 40, 62, 84, 106, 126, 162),
        scoreSimp = function(quest) return floor(quest) .. " Quests" end,
        hide = function() return GAME.completion.DH == 0 end,
        credit = "@Flowerling",
    },
    { -- muga
        id = 'sw_muga',
        name = "Non-Self",
        desc = [[HFD with VL DH rIN rDP and Redaction (R)]],
        quote = [["Erasing you, erasing me, and everything for eternity."]],
        rank = floorRank(1, 3, 4, 5, 6, 7, 8),
        hide = function() return GAME.completion.IN == 0 and GAME.completion.DP == 0 end,
    },
    { -- Also play Super Checkpoint!
        id = 'sw_checkpoint1',
        name = "Curse of the Perfect Run",
        desc = [[HFD with DP and No-Hit (P)]],
        quote = [[A proven devotion to the Dream Space Trials.]],
        rank = floorRank(1, 3, 5, 7, 9, 10, 2000),
    ...
    { -- flavored one :tf:
        id = 'sw_niflhel',
        name = "The Unfavorable One",
        desc = [[HAR with rMS rVL DH and Graceless (G)]],
        quote = [["Even when you're gone..."]],
        rank = floorRank(1, 3, 5, 7, 9, 10, 2600),
        hide = function() return GAME.completion.VL == 0 and GAME.completion.MS == 0 end,
    },
    { -- angel
        id = 'sw_angelicide',
        name = "73911@",
        desc = [[Highest CSP reached with rNH MS DH rAS and Graceless (G)]],
        quote = [[MSG('dark', "you are an angel |!'!|<3|!''!|", 2.6)]],
        scoreSimp = function(rank) return "Rank " .. rank end,
        rank = numberRank(6, 7, 7, 8, 8, 9, 10),
        hide = function() return GAME.completion.AS == 0 and GAME.completion.NH == 0 end,
    },
